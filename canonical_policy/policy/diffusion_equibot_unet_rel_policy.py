...
from canonical_policy.model.diffusion.equibot_conditional_unet1d_rel import VecConditionalUnet1DRel
...
from canonical_policy.model.vision.equibot_extractor import EquibotEncoder
    
class EquibotRel(BaseImagePolicy):
    def __init__(self, 
            shape_meta: dict,
        ...
        obs_encoder = EquibotEncoder(observation_space=obs_dict,
                                    equibot_encoder_cfg=equibot_encoder_cfg,
        ...
        if obs_as_global_cond:
            input_dim = 3  # pos + dir1 + dir2
            global_cond_dim = obs_feature_dim * n_obs_steps
            scalar_cond_dim = 2 * n_obs_steps  # robot_state_gripper * To
            scalar_input_dim = 1  # for gripper scalar

        self.use_pc_color = use_pc_color
        self.pointnet_type = pointnet_type
        cprint(f"[DiffusionEquibotPolicy] pointnet_type: {self.pointnet_type}", "yellow")

        model = VecConditionalUnet1DRel(
            input_dim=input_dim,
    ...
    def conditional_sample(self, 
        ...
        for t in scheduler.timesteps:
            ...
            model_output = self.model(sample=trajectory, 
                          timestep=t, 
                          obs_cond_vec=obs_cond_vec, 
                          obs_cond_scalar=obs_cond_scalar,
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        nsample = self.conditional_sample(
            cond_data, 
            cond_mask,
            obs_cond_vec=ret['obs_cond_vec'],
            obs_cond_scalar=ret['obs_cond_scalar'],
            points_scale=ret['points_scale'],
            **self.kwargs)
