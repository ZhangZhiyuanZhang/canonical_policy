...
class VecConditionalUnet1DRel(nn.Module):
    def __init__(
        self,
        input_dim,
        cond_dim,
    ...
    def forward(
        ...
        **kwargs
        ...
        sample_pos, sample_rpy, sample_gripper = sample[..., :3], sample[..., 3:6], sample[..., [-1]]  # [B, T, 3], [B, T, 3], [B, T, 1]
        sample_quat = axis_angle_to_quaternion(sample_rpy)  # [B, T, 4]
        sample_sixd = self.get6DRotation(sample_quat)  # [B, T, 6]
        sample_dir1, sample_dir2 = sample_sixd[..., :3], sample_sixd[..., 3:]

        sample_pos = sample_pos / points_scale   # [B, T, 3]
        ...
        x_scalar = einops.rearrange(x_scalar, "b t h -> b h t") # [B, T, 1]
        ...
        x_pos = x_pos * points_scale  # [B, T, 3]
        x_sixd = torch.cat([x_dir1, x_dir2], dim=-1)   # [B, T, 6]
        x_quat = self.getQuat(x_sixd)   # [B, T, 4]
        x_rpy = quaternion_to_axis_angle(x_quat)    # [B, T, 3]

        x = torch.cat((x_pos, x_rpy, x_scalar), dim=-1) # [B, T, 7]
