...
class TrainEquibotWorkspace:
    ...
    def run(self):
        ...
        while self.epoch < cfg.training.num_epochs:
            ...
            # run rollout
            if ((self.epoch % cfg.training.rollout_every) == 0 and self.epoch is not 0 and RUN_ROLLOUT and env_runner is not None) or (self.epoch > (cfg.training.num_epochs-10)):
                t3 = time.time()
...
def main(cfg):
    workspace = TrainEquibotWorkspace(cfg)
    workspace.run()
